...
myWin = visual.Window(color=[0,0,0], units='pix', size=[1000,1000], allowGUI=False, fullscr=False)#creates a window 
myClock = core.Clock() #this creates and starts a clock which we can later read

movingLilac = visual.Circle(myWin, radius=26, fillColor=[0.15, 0, 0.15], lineColor=None)
...
def mainLoop(radius=160):
    ...
    while not finished:
        ...
        waitUntil = myClock.getTime() + 0.166
